...
app = modal.App("ai-podcast", image=image)

volume = modal.Volume.from_name(
    "ai-podcast-model-cache", create_if_missing=True)
...
def create_vertical_video(tracks, scores, pyframes_path, pyavi_path, audio_path, output_path, framerate=25):
    ...
    for fidx, fname in tqdm(enumerate(flist), total = len(flist), desc="Creating vertical video"):
        ...
        if mode == "resize":
            ...
            image_cropped = resized_image[
                0:target_height, top_x:top_x + target_width
            ]
...
def process_clip (base_dir: str, original_video_path: str, s3_key: str, start_time: float, end_time: float, clip_index: int, transcript_segments: list):
    ...
    vertical_mp4_path = clip_dir/"pyavi" / "vertical.mp4"
    subtitle_output_path = clip_dir / "pyavi" / "subtitled.mp4"
    ...
    cut_command = f"ffmpeg -i {original_video_path} -ss {start_time} -t {duration} -c copy {clip_segment_path}"
    ...
    if not tracks_path.exists() or not scores_path.exists():
        raise FileNotFoundError("Columbia script did not produce expected output files.")
    ...
    print(f"{clip_index} clip completed in {cvv_end_time - cvv_start_time:.2f} seconds")
    ...
    s3_client.upload_file(subtitle_output_path, "ai-podcast-clipperss", output_s3_key)
...
class AiPodcastClipper:
    ...
    def transcribe_video(self, base_dir: str, video_path: str) -> str:
        ...
        print("Transcription took {:.2f} seconds".format(duration))
    ...
    def identify_moments(self, transcript: dict):
        response = self.gemini_client.models.generate_content(model = "gemini-2.5-flash-lite-preview-06-17", contents = """ 
    ...
    def process_video(self, request: ProcessVideoRequest, token: HTTPAuthorizationCredentials = Depends(auth_scheme)):
        ...
        s3_client = boto3.client('s3')
        s3_client.download_file("ai-podcast-clipperss", s3_key, str(video_path))
        ...
        if not clip_moments or not isinstance(clip_moments, list):
            print("Error: Viral moments is not a list")
            clip_moments = []
